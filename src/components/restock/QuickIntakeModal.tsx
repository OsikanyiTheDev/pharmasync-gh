...
export const QuickIntakeModal: React.FC<QuickIntakeModalProps> = ({ isOpen, product, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#222327] border border-slate-100/60 dark:border-white/5 rounded-2xl max-w-md w-full p-5 text-slate-900 dark:text-white shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative animate-in fade-in zoom-in-95">
        ...
        <div className="flex items-center space-x-2.5 border-b border-slate-100 dark:border-white/5 pb-3 mb-3">
          <div className="p-2 bg-[#FBBF24]/20 text-[#F59E0B] rounded-xl border border-[#FBBF24]/30">
            <Truck className="w-5 h-5" />
          </div>
          <div>
            <h3 className="font-bold text-base text-slate-900 dark:text-white">Okaishie Market Stock Intake</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Record Wholesale Purchase directly to Branch</p>
          </div>
        </div>

        <div className="bg-[#F3F4F7] dark:bg-[#161719] p-3 rounded-xl border border-slate-100 dark:border-white/5 mb-3 space-y-1">
        ...
        <div className="space-y-3">
          <div>
            ...
            <select value={targetBranch} onChange={(e) => setTargetBranch(e.target.value as BranchId)} className="w-full p-2.5 bg-[#F3F4F7] dark:bg-[#161719] border border-slate-100 dark:border-white/10 rounded-xl text-xs font-bold text-[#4E60FF] focus:outline-none focus:ring-2 focus:ring-[#4E60FF]">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="number" min="1" value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value) || 1)} className="w-full p-2.5 bg-[#F3F4F7] dark:bg-[#161719] border border-slate-100 dark:border-white/10 rounded-xl text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#4E60FF]" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block mb-1">Unit Cost (GH₵)</label>
              <input type="number" value={wholesaleCost} onChange={(e) => setWholesaleCost(parseFloat(e.target.value) || 0)} className="w-full p-2.5 bg-[#F3F4F7] dark:bg-[#161719] border border-slate-100 dark:border-white/10 rounded-xl text-sm font-bold text-[#F59E0B] focus:outline-none focus:ring-2 focus:ring-[#4E60FF]" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="text" value={batchNumber} onChange={(e) => setBatchNumber(e.target.value)} className="w-full p-2.5 bg-[#F3F4F7] dark:bg-[#161719] border border-slate-100 dark:border-white/10 rounded-xl text-xs font-mono font-bold text-[#4E60FF] focus:outline-none focus:ring-2 focus:ring-[#4E60FF]" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-700 dark:text-slate-300 block mb-1">Expiry Date</label>
              <input type="date" value={expiryDate} onChange={(e) => setExpiryDate(e.target.value)} className="w-full p-2.5 bg-[#F3F4F7] dark:bg-[#161719] border border-slate-100 dark:border-white/10 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#4E60FF]" />
            </div>
          </div>
