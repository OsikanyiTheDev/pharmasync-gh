...
export const ProductSearch: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-[#222327] rounded-2xl border border-slate-100/60 dark:border-white/5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 text-slate-900 dark:text-white flex flex-col space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="block w-full pl-11 pr-24 py-3 bg-[#F3F4F7] dark:bg-[#161719] border border-slate-200/80 dark:border-white/10 rounded-xl text-sm font-medium text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#4E60FF] focus:border-transparent transition-all"
        />
        <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
          <kbd className="hidden sm:inline-block px-2 py-0.5 text-[10px] font-mono font-bold bg-white dark:bg-[#222327] border border-slate-100 dark:border-white/10 text-[#4E60FF] rounded-lg shadow-xs">
            Press /
          </kbd>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 text-xs">
        {categories.map(cat => (
          <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-3.5 py-2 rounded-xl font-medium transition-all whitespace-nowrap cursor-pointer ${selectedCategory === cat ? 'bg-[#4E60FF] text-white shadow-sm font-semibold' : 'bg-[#F3F4F7] text-slate-600 hover:bg-slate-100 dark:bg-white/5 dark:text-slate-400 dark:hover:text-white'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[560px] overflow-y-auto pr-1">
        {filteredProducts.length === 0 ? (
          <div className="col-span-2 py-12 text-center text-slate-400 bg-[#F3F4F7] dark:bg-[#161719] rounded-xl border border-dashed border-slate-200 dark:border-white/10">
          ...
          filteredProducts.map(product => {
            ...
            return (
              <div key={product.id} onClick={() => handleAddProduct(product)} className={`p-3.5 rounded-xl border transition-all cursor-pointer flex flex-col justify-between group shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] ${totalStock === 0 ? 'bg-[#F3F4F7] dark:bg-[#161719] border-slate-100 dark:border-white/5 opacity-60' : 'bg-white dark:bg-[#222327] border-slate-100/60 dark:border-white/5 hover:border-[#4E60FF]/50'}`}>
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-[#F3F4F7] dark:bg-[#161719] text-slate-600 dark:text-slate-400 border border-slate-100 dark:border-white/5">
                  ...
                  <h3 className="font-bold text-sm text-slate-900 dark:text-white group-hover:text-[#4E60FF] transition-colors">{product.brandName}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1 mt-0.5">{product.genericName}</p>
                </div>
                <div className="mt-3 pt-2.5 border-t border-slate-100 dark:border-white/5 flex items-center justify-between">
                  <div>
                    {fefoBatch ? (
                      <span className={`text-[10px] font-medium flex items-center space-x-1 ${expiryStatus?.status === 'EXPIRED' ? 'text-[#EF4444]' : expiryStatus?.status === 'EXPIRING_SOON' ? 'text-[#F59E0B] font-bold' : 'text-slate-500 dark:text-slate-400'}`}>
