...
import { X, Banknote, CreditCard, Smartphone, Split, ArrowRight, User } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white border border-slate-100/60 rounded-2xl max-w-lg w-full p-6 text-slate-900 shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative animate-in fade-in zoom-in-95">
        ...
        <div className="p-4 bg-[#4E60FF] text-white rounded-2xl mb-4 flex items-center justify-between shadow-sm">
          <div>
            <p className="text-[10px] text-white/80 uppercase font-bold tracking-wider">Total Amount Due</p>
            <p className="text-3xl font-bold tabular-nums">GH₵ {cartTotal.toFixed(2)}</p>
          </div>
          <div className="text-right">
            <span className="px-2.5 py-1 bg-white/20 rounded-lg text-xs font-bold text-white border border-white/20">
              {paymentMethod}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          ...
          <button
            onClick={() => { setPaymentMethod('MOMO_MTN'); setMomoProvider('MTN Mobile Money'); }}
            className={`p-2.5 rounded-xl border text-center font-bold text-xs flex flex-col items-center justify-center transition-all ${
          ...
          <button
            onClick={() => { setPaymentMethod('MOMO_TELECEL'); setMomoProvider('Telecel Cash'); }}
            className={`p-2.5 rounded-xl border text-center font-bold text-xs flex flex-col items-center justify-center transition-all ${
          ...
          {paymentMethod === 'CASH' && (
            <div className="space-y-2">
              ...
              <div className="p-3 bg-[#10B981]/10 border border-[#10B981]/20 rounded-xl flex items-center justify-between">
                <span className="text-[#10B981] font-bold">Change Due:</span>
                <span className="text-xl font-bold text-[#10B981] tabular-nums">GH₵ {cashChange.toFixed(2)}</span>
              </div>
            </div>
          )}
