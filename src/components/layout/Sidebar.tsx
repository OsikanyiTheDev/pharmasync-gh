...
export const Sidebar: React.FC<SidebarProps> = ({ onOpenShortcuts }) => {
  ...
  return (
    <>
      <div className="md:hidden sticky top-0 z-40 bg-white text-slate-900 border-b border-slate-100 px-4 py-3 flex items-center justify-between shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
      ...
      {isMobileOpen && <div onClick={() => setIsMobileOpen(false)} className="md:hidden fixed inset-0 bg-black/20 backdrop-blur-sm z-40" />}

      <aside className={`fixed top-0 left-0 bottom-0 w-64 bg-white border-r border-slate-100 flex flex-col justify-between z-30 transition-all duration-200 ${isMobileOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full md:translate-x-0'}`}>
