...
export const KeyboardModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white border border-slate-100/60 rounded-2xl max-w-lg w-full p-6 text-slate-900 shadow-[0_8px_30px_rgb(0,0,0,0.08)] relative animate-in fade-in zoom-in-95 duration-150">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-700 p-1 rounded-lg hover:bg-slate-50"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex items-center space-x-3 border-b border-slate-100 pb-4 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Ultra-fast checkout hotkeys for pharmacy attendants</p>
          </div>
        </div>

        <div className="space-y-2.5">
          {shortcuts.map((s, idx) => (
            <div key={idx} className="flex items-center justify-between p-2.5 bg-[#F3F4F7] rounded-xl border border-slate-100">
              <span className="text-xs font-medium text-slate-700">{s.description}</span>
              <kbd className="px-2.5 py-1 bg-white border border-slate-100 text-[#4E60FF] rounded-lg font-mono text-xs font-bold shadow-xs">
                {s.key}
              </kbd>
            </div>
          ))}
