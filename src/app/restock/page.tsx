...
export default function RestockPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 text-slate-900 dark:text-white pb-24">
      
      {/* Clinical Market Banner - Floating White Card with Brand Accent */}
      <div className="bg-white dark:bg-[#222327] border border-slate-100/60 dark:border-white/5 p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] space-y-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h1 className="text-lg font-bold text-slate-900 dark:text-white">Wholesale Market Restock Assistant</h1>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-4 bg-[#F3F4F7] dark:bg-[#161719] rounded-xl border border-slate-100 dark:border-white/5 flex items-center space-x-3">
            <div className="bg-[#10B981] text-white p-3 rounded-full shadow-sm">
              <DollarSign className="w-5 h-5" />
            </div>
            <div>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Est. Capital Required</p>
              <p className="text-xl font-bold text-slate-900 dark:text-white tabular-nums">GH₵ {totalMarketOutlay.toFixed(2)}</p>
            </div>
          </div>
          <div className="p-4 bg-[#F3F4F7] dark:bg-[#161719] rounded-xl border border-slate-100 dark:border-white/5 flex items-center space-x-3">
            ...
            <div>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold tracking-wider">Progress</p>
              <p className="text-sm font-bold text-slate-900 dark:text-white">{itemsBoughtCount} / {restockItems.length} Bought</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{restockItems.length} Items to Buy</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {restockItems.length === 0 ? (
          <div className="bg-white dark:bg-[#222327] border border-slate-100/60 dark:border-white/5 rounded-2xl p-8 text-center space-y-3 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            <div className="bg-[#10B981] text-white p-3.5 rounded-full w-fit mx-auto shadow-sm">
              <CheckCircle2 className="w-8 h-8" />
            </div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">All Branches Fully Stocked!</h3>
            ...
            return (
              <div
                ...
                className={`p-5 rounded-2xl border transition-all cursor-pointer shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col space-y-3 ${
                  isChecked
                    ? 'bg-[#10B981]/5 dark:bg-[#10B981]/10 border-[#10B981]/30 dark:border-[#10B981]/30 opacity-70'
                    : 'bg-white dark:bg-[#222327] border-slate-100/60 dark:border-white/5 hover:border-[#4E60FF]/30'
                }`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <h3 className={`font-bold text-sm ${isChecked ? 'line-through text-slate-500 dark:text-slate-400' : 'text-slate-900 dark:text-white'}`}>
                        {p.brandName}
                      </h3>
                ...
                <div className="grid grid-cols-3 gap-2 bg-[#F3F4F7] dark:bg-[#161719] p-2.5 rounded-xl border border-slate-100 dark:border-white/5 text-center text-xs">
                  <div>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold">Accra Central</p>
                    <p className={`font-bold tabular-nums ${item.branchBreakdown.ACCRA_MAIN <= 5 ? 'text-[#EF4444]' : 'text-slate-800 dark:text-slate-200'}`}>
                      {item.branchBreakdown.ACCRA_MAIN} pk
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold">Osu Branch</p>
                    <p className={`font-bold tabular-nums ${item.branchBreakdown.OSU_BRANCH <= 5 ? 'text-[#EF4444]' : 'text-slate-800 dark:text-slate-200'}`}>
                      {item.branchBreakdown.OSU_BRANCH} pk
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold">Spintex Road</p>
                    <p className={`font-bold tabular-nums ${item.branchBreakdown.SPINTEX_BRANCH <= 5 ? 'text-[#EF4444]' : 'text-slate-800 dark:text-slate-200'}`}>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-white/5">
                  <div>
                    <p className="text-xs text-slate-600 dark:text-slate-400">Buy Rec: <b className="text-[#4E60FF] font-bold tabular-nums">{item.recommendedQty} packs</b></p>
                    <p className="text-xs text-slate-600 dark:text-slate-400">Est Cost: <b className="text-[#10B981] font-bold tabular-nums">GH₵ {item.totalEstimatedCost.toFixed(2)}</b></p>
                  </div>
                  <button
